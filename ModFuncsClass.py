...
class device():
    ...
    def read_holding_floats(self, x): # for the multi reads x degeri read() fonksiyonundaki 
        if self.conn.is_socket_open():
            try:
                self.rr = self.conn.read_holding_registers(self.reg[x], self.cnt[x] * 2, unit=self.address)
                if hasattr(self.rr, 'registers'):
                    ...
                    self.data = [self.TempData[i] for i in self.indexes[x]]
                else:
                    self.rr = self.conn.read_holding_registers(self.reg[x], self.cnt[x] * 2, unit=self.address)
                    if hasattr(self.rr, 'registers'):
                        self.decoder = BinaryPayloadDecoder.fromRegisters(self.rr.registers, Endian.Big, wordorder=Endian.Big)
                        for a in range(0, self.cnt):            
                            self.TempData.append(round(self.decoder.decode_32bit_float(), 2))
                        self.data.append([self.TempData[i] for i in self.indexes[x]]) #a list comprehantion.we may use even an if clause.
            except CError:
